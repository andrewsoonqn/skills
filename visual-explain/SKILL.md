...
name: visual-explain
description: >-
  Use this skill whenever a topic, concept, system, mechanism, workflow, comparison, or relationship
  would be clearer as a visual explanation instead of prose. It renders an explanation as MDX to a
  scannable, self-contained HTML page with diagrams, timelines, tables, charts, and callouts via the
  `vplan` CLI. Apply it when the user says "explain visually", "show how this works", "make a visual
  explainer", "diagram this", or asks to see an explanation. Skip it when prose is already the
  clearest form or the user explicitly asks for plain prose.
---

## Purpose

Render an explanation as a visual MDX page instead of a wall of text, using the `vplan` CLI. The
component vocabulary is general: although the examples below are code-flavored, it fits any
structured explanation. Use the components that fit and skip the ones that do not.

Above all, **show, don't tell**: a reader should grasp the subject by scanning its diagrams and
tables, with prose only connecting the visuals, not carrying the explanation itself. The
...
- `<Phase title="..." status="planned|active|done">`: one step in a numbered vertical
  ...
  order. Use it when sequence or progression is part of the explanation.
...
- `<Stat>`: headline facts or metrics as a grid of cards. One card per bullet,
  ...
  Use this for static facts, not time series (use `<Chart>` for those). Only add a `<Stat>` when the
  explanation genuinely has standout facts worth surfacing; an invented or filler metric is worse
  than omitting the component entirely.
...
- Mark lines and text inside a code block with Expressive Code props in the fence meta string
  ...
  `/regex/`. Use this to call attention to the lines the explanation discusses.
...
The whole point of a visual explanation is to replace a wall of prose with something the reader
grasps by scanning. Default to a component over a sentence: if a fact has structure, show it; do not
describe it in paragraphs. Prose is the connective tissue between visuals, never the substance.

- **Lead with the structure.** Open with at most a one-paragraph context, then the diagram, table, or
  other visual that exposes the subject's shape. The reader should understand that shape before
  reading a full explanation.
- **Prefer a diagram or a `<FileTree>` to describing structure in words.** A flowchart of a data path
  beats a paragraph tracing it; a file map beats sentences listing the files.
- **Move the meaning out of prose into the component that carries it.** Risks and decisions go in
  `<Callout>`s, unresolved points in `<Questions>`, and tradeoffs in `<Compare>` or `<Matrix>`, not
  buried in paragraphs where they are easy to skim past.
- **Keep prose tight inside phases.** A `<Phase>` is a step, not an essay: a line or two of context,
  then the visual. The visual is the point.
- **Right-size what you show.** Do not add a diagram or phase that carries no information: an empty
  two-node flowchart shows nothing and is worse than one plain sentence. Show when there is structure
  to show; otherwise a tight sentence is fine. This applies to `<Stat>` too, as its own entry notes.
...
- **Never pass `--no-open` for a user-facing explanation.** The point is that the user sees it.
  Reserve `--no-open` for an explicit headless/CI request.
...
commented `FileTree` move row, a wildly-scaled `Chart`) are enforced by the `check` quality lint and
will fail it, so they are hard rules, not just style advice.
